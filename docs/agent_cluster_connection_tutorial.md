...
另外，历史 agent 会话的记录显示，之前曾直接运行过
`scripts/remote_yundun.py --node 3` 并成功完成训练部署、脚本上传和诊断。那次会话中的 `CLUSTER_PWD` 已经存在于该会话的进程环境（具体是由更早的交互设置还是平台继承，当前无法回溯），而新会话从全新的进程环境启动，因此变量为空。会话级环境变量不会自动跨任务、跨窗口或跨应用继承。
