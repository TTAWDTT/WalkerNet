...
示例：
    python scripts/diagnostics/plot_rollout_spatial_diagnostics.py \
...
def plot_map_diagnostics(
    ...
    display_name = "SSTA" if variable == "tos" else f"{variable} anomaly"
    file_prefix = "ssta" if variable == "tos" else variable

    fig.suptitle(f"{display_name} spatial comparison | {sample_title}", y=0.995, fontsize=10, fontweight="bold")
    fig.tight_layout()
    fig.savefig(output_dir / f"{file_prefix}_forecast_target_error_maps.png")
    fig.savefig(output_dir / f"{file_prefix}_forecast_target_error_maps.pdf")
    plt.close(fig)
...
def plot_grid_acc(
    ...
    fig.suptitle(f"{display_name} grid-point ACC over test rollout", y=0.995, fontsize=10, fontweight="bold")
    fig.savefig(output_dir / f"{file_prefix}_gridpoint_acc.png", bbox_inches="tight")
    fig.savefig(output_dir / f"{file_prefix}_gridpoint_acc.pdf", bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    ...
    for batch_idx, batch in enumerate(loader, start=1):
        ...
        if batch_idx - 1 == args.sample_position:
            ...
            year = int(dataset.source_payloads[int(source_index[0].detach().cpu().item())]["years"][target_t])
            month = int(dataset.source_payloads[int(source_index[0].detach().cpu().item())]["months"][target_t])
            sample_title = f"{source_name}, target={year:04d}-{month:02d}, subset_position={args.sample_position}"
            sample_metadata = "\n".join(
                [
                    f"source={source_name}",
                    f"split={args.split}",
                    f"subset_position={args.sample_position}",
                    f"target_t={target_t}",
                    f"target_year_month={year:04d}-{month:02d}",
                    f"variable={args.variable}",
                    f"map_leads={','.join(str(lead) for lead in map_leads)}",
                    f"acc_leads={','.join(str(lead) for lead in leads)}",
    ...
    np.savez_compressed(
        output_dir / f"{'ssta' if args.variable == 'tos' else args.variable}_gridpoint_acc_maps.npz",
        **{f"lead_{lead}": acc_maps[lead] for lead in leads},
        lat=np.asarray(dataset.lat),
        lon=np.asarray(dataset.lon),
    )
    (output_dir / "sample_metadata.txt").write_text(sample_metadata + "\n", encoding="utf-8")
