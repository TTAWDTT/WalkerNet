...
launch_job() {
  local branch="$1" source="$2" year="$3" gpu="$4"
  local mode=lead_delta
  [[ "$branch" == delayed ]] && mode=delayed_lead_delta
  local slug="$(slugify "$source")_${year}"
  local job_dir="$OUT/$branch/$slug"
  local log="$LOGDIR/${branch}_${slug}.log"
  mkdir -p "$job_dir"
  echo "[$(date -Is)] start branch=$branch source=$source year=$year gpu=$gpu" >> "$log"
...
echo "[$(date -Is)] launcher start host=$(hostname)" | tee "$LOGDIR/launcher.log"
while IFS=, read -r source year target_t observed; do
  [[ "$source" == source || -z "$source" ]] && continue
  for branch in normal delayed; do
    gpu=$((job_count % ${#GPUS[@]}))
    ...
    launch_job "$branch" "$source" "$year" "${GPUS[$gpu]}"
  done
done < "$MANIFEST"
