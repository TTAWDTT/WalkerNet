...
def decoder_rollout() -> Path:
    arch = [
        ...
        # not the low-resolution token tensor.  A separate plain node keeps
        # this branch readable without adding another oversized 3-D block.
        r"\tikzset{basebox/.style={draw=black!45,fill=yellow!15,rounded corners=2pt,align=center,font=\scriptsize,minimum width=2.2cm,minimum height=0.65cm,inner sep=3pt}}",
        r"\node[basebox] (xlast) at ($(add-south)+(-0.4,-1.50)$) {$x_{last}$\\last state};",
        r"\draw [copyconnection,-Stealth] (xlast.east) .. controls +(1.0,0) and +(-0.4,-1.0) .. (add-south);",
        ...
        r"\tikzset{rollbox/.style={draw=black!45,fill=blue!5,rounded corners=2pt,align=center,font=\scriptsize,minimum width=2.35cm,minimum height=0.85cm,inner sep=3pt}}",
        r"\node[font=\small\bfseries,text=black!60] at (8.0,-3.55) {Autoregressive rollout};",
        r"\node[rollbox] (w0) at (1.5,-4.65) {History window\\$x_{t-11:t}$};",
        r"\node[rollbox] (m1) at (4.3,-4.65) {WalkerNet\\step 1};",
        r"\node[rollbox] (p1) at (7.1,-4.65) {Prediction\\$\hat{x}_{t+1}$};",
        r"\node[rollbox] (w1) at (9.9,-4.65) {Shift window\\append / drop};",
        r"\node[rollbox] (m2) at (12.7,-4.65) {WalkerNet\\step 2};",
        r"\node[rollbox] (p2) at (15.5,-4.65) {Prediction\\$\hat{x}_{t+2}$};",
        r"\node[font=\Large\bfseries] (more) at (18.0,-4.65) {$\cdots$};",
