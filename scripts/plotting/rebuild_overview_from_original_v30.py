...
OUT = ROOT / "docs" / "assets" / "article" / "pacific_delay_overview_original_layout_v30.png"
DESKTOP_OUT = Path(r"C:\Users\zhen.luo\Desktop\article\overview_original_layout_v30.png")
...
MAP_HEIGHT = 434
# This is the original script's shared MAP_BOX.  The first visible x tick is
# 120E, but the panel itself begins at 100E.
...
def main() -> None:
    ...
    # Copy the complete case-name strip, then clear the separate tick gutter;
    # this preserves full names without leaking old axis glyphs.
    canvas.paste(Image.fromarray(base[:, :450]), (0, 0))
    canvas.paste(Image.fromarray(base[:230, 450:]), (450, 0))
    ...
    for row, case in enumerate(CASES):
        ...
        if row == len(CASES) - 1:
            ...
            y = top + MAP_HEIGHT + 8
            tick_lons = (120.0, 150.0, 180.0, 210.0, 240.0, 270.0, 300.0)
            for label, tick_lon in zip(labels, tick_lons):
                x = int(round(LEFT_X0 + (tick_lon - LON_MIN) / (LON_MAX - LON_MIN) * (LEFT_X1 - LEFT_X0 - 1)))
    ...
    draw = ImageDraw.Draw(canvas)
    draw.rectangle((450, 230, 635, HEIGHT), fill="white")
    draw.rectangle((450, 55, 635, 245), fill="white")
